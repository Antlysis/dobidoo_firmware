...
#define SHIFT_IN_LATCH 16      //JP3 (second two)
#define SHIFT_IN_CLOCK 4      //JP5 (third two
...
#define MQTT_SERVER      "192.168.0.112" //"192.168.1.102" // give static address
...
const char* fwUrlBase = "http://192.168.0.112/fw/";

// Create an ESP8266 WiFiClient class to connect to the MQTT server. 
WiFiClient client; 
 //Setup the MQTT client class by passing in the WiFi client and MQTT server and login details. 
Adafruit_MQTT_Client mqtt(&client, MQTT_SERVER, MQTT_PORT, MQTT_USERNAME, MQTT_PASSWORD); 
Adafruit_MQTT_Publish lastwill = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "connectivity/4ed12e0e2672bda7718278f7cf25df06");

// Notice MQTT paths for AIO follow the form: <username>/feeds/<feedname> 
Adafruit_MQTT_Publish runStatus = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "Lock/4ed12e0e2672bda7718278f7cf25df06");
Adafruit_MQTT_Publish detStatus = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "detDrop/4ed12e0e2672bda7718278f7cf25df06");
Adafruit_MQTT_Publish coinIn = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "coinIn/4ed12e0e2672bda7718278f7cf25df06");
Adafruit_MQTT_Publish billIn = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "billIn/4ed12e0e2672bda7718278f7cf25df06");
Adafruit_MQTT_Publish versFeed = Adafruit_MQTT_Publish(&mqtt, MQTT_USERNAME "versionFeed/4ed12e0e2672bda7718278f7cf25df06");
// Setup a feed called 'esp8266_led' for subscribing to changes. 
Adafruit_MQTT_Subscribe receivedPayment = Adafruit_MQTT_Subscribe(&mqtt, MQTT_USERNAME "4ed12e0e2672bda7718278f7cf25df06");
...
int dsCounter1 = 0;
int blchCounter2 = 0;
uint8_t turn_drySoft = 0;
uint8_t turn_blch2 = 0;
int begCounter = 0;
uint8_t turn_beg = 0;
int coin_input1 = 0;
int coin_input2 = 0;
...
void IOtimerISR(void *pArg) {
  ...
  if (machineType == "ipsoWasher") {
    ...
    coinCount(coin_input1, &coinH, &high1, &coinL);
    ...
    coin_input1 = digitalRead(DEX_D_IN);
    coinCount(coin_input1, &coinH, &high1, &coinL);
  } else if (machineType == "vendingBill") {
    BillReadCoin(&register_value, &high1, &billCounter);
  }
}
...
void setup() {
 ...
 mqtt.will(MQTT_USERNAME "connectivity/4ed12e0e2672bda7718278f7cf25df06", "OFF");
 timerInit();
...
void loop() { 
 ...
 // connection and automatically reconnect when disconnected).  See the MQTT_connect 
  ...
  if (machineType == "ipsoWasher") {
    ...
    //lockState1 =  digitalRead(IPSO_D_STATUS);
    //coin_input1 = digitalRead(IPSO_D_IN);
    //coinCountD(coin_input1, &coinH, &high1, &coinL);
    ipsoDryerStatusRead(lockState1, &SendOne1, &checkOne1, &lockCounter1, &unlockCounter1);
    while ((subscription = mqtt.readSubscription())) { 
      if (subscription == &receivedPayment) {
        char *message = (char *)receivedPayment.lastread; 
    ...
    DetReadStatus(&register_value, &turn_sft, &turn_beg, &turn_drySoft, &turn_blch2,  &turn_dtg1, &turn_dtg2, &sftnrCounter, &begCounter, &dtgCounter1, &dtgCounter2, &dsCounter1, &blchCounter2);
    detergentCoinCount(coin_input1, coin_input2, &high1, &high2, &coinCounter1, &coinCounter2);
    BillReadCoin(&register_value, &billhigh1, &billCounter);
...
void coinCountD(int coin_input, int *coinHigh, int *high, int *coinLow) {   
  if(*high == 0) {
    if(coin_input == HIGH) {
      //Serial.print("coin high");
      (*coinHigh)++; 
...
void BillReadCoin(uint8_t *shiftReg, int *high1, int *billCounter1 ) {
  uint8_t compbill = *shiftReg & 0b00000010;
  //coinIn.publish(*shiftReg);
  if (*high1 == 0) {
    if(compbill == 0) {
      (*billCounter1)++;
      if (*billCounter1 > 1) {
...
void DetReadStatus(uint8_t *shiftReg, uint8_t *turn_sft, uint8_t *turn_beg, uint8_t *turn_drySoft, uint8_t *turn_bleach2,  uint8_t *turn_dtg1, uint8_t *turn_dtg2, int *sftnrCounter, int *begCounter, int *dtgCounter1, int *dtgCounter2, int *dsCounter1, int *blchCounter2) {
  uint8_t compSoft =            *shiftReg & 128;
  uint8_t compOxyBleach =       *shiftReg & 64; //done
  uint8_t compDrySoft =         *shiftReg & 32; //done
  uint8_t compLB =               *shiftReg & 16; //done
  uint8_t compDet1 =            *shiftReg & 8; //done
  uint8_t compDet2 =              *shiftReg & 4;
  //uint8_t compbill =      *shiftReg & 0b00000010;
  //delay(1000);
...
//// Compare 5th slot for Bleach 2 ////////
  ...
  } else if (*turn_bleach2 == 1) {
    if (compOxyBleach == 0) {
      (*blchCounter2)++;
      if (*blchCounter2 >20) {
        *turn_bleach2 = 0;
        detStatus.publish("OXYBLCH_DROP");  
      } 
    } else {
      *turn_bleach2 = 1;
    }
